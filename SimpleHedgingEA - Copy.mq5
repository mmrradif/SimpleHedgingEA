...
//|                                             https://www.mql5.com |
//| Description: Progressive Distance Spacing Grid EA (0.10 to 1.00) |
//+------------------------------------------------------------------+
#property copyright "Copyright 2026, Antigravity AI"
#property link      "https://www.mql5.com"
#property version   "26.00"
#property description "Progressive Distance Pending Grid EA (10 Orders: 0.10 to 1.00 Lot)"
...
input double   InpLotStep             = 0.10;     // Lot Increment Step (0.10)
input double   InpMaxLotLimit         = 1.00;     // Max Lot Limit (1.00) - Exactly 10 Orders
...
int OnInit()
{
   ...
   PrintFormat("[INIT] Progressive Distance Grid EA v26.0 Initialized. 10 Orders: 0.10 to 1.00 Lot, Target: $%.2f", 
               InpTargetProfitUSD);
...
void OnTick()
{
   ...
   // 4. GUARANTEED PROFIT EXIT -> CLOSE ALL POSITIONS & DELETE ALL PENDINGS -> RESET
...
//+------------------------------------------------------------------+
//| Setup Progressive Spacing Pending Grid (10 Orders: 0.10 -> 1.00) |
//+------------------------------------------------------------------+
void SetupProgressivePendingGrid()
{
   ...
   double startLot = (InpStartLot > 0) ? InpStartLot : 0.10;
   double lotStep = (InpLotStep > 0) ? InpLotStep : 0.10;
   double maxLot = (InpMaxLotLimit >= startLot) ? InpMaxLotLimit : 1.00;
   ...
   // 1. Place 10 BUY STOP Orders (0.10, 0.20, 0.30 ... 1.00)
   for(int i = 1; i <= stepCount; i++)
   {
      double lot = NormalizeLot(startLot + (i - 1) * lotStep);
      double price = NormalizeDouble(buyBasePrice + cumulativeBuyOffset, _Digits);

      if(m_trade.BuyStop(lot, price, _Symbol, 0, 0, ORDER_TIME_GTC, 0, "BuyStop 0.10-1.00"))
      {
   ...
   // 2. Place 10 SELL STOP Orders (0.10, 0.20, 0.30 ... 1.00)
   for(int i = 1; i <= stepCount; i++)
   {
      double lot = NormalizeLot(startLot + (i - 1) * lotStep);
      double price = NormalizeDouble(sellBasePrice - cumulativeSellOffset, _Digits);

      if(m_trade.SellStop(lot, price, _Symbol, 0, 0, ORDER_TIME_GTC, 0, "SellStop 0.10-1.00"))
      {
